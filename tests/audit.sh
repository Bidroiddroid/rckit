...
if grep -R -n -E 'Install placeholder|Configure placeholder|Update placeholder|installer is not implemented' "$ROOT_DIR/lib" "$ROOT_DIR/modules"; then
  echo "functional placeholder found" >&2
  exit 1
fi

for name in postgresql redis mysql traefik portainer; do
  grep -q "service_verify $name" "$ROOT_DIR/modules/$name/module.sh"
...
grep -q 'opencode.json.*raiz' "$ROOT_DIR/README.md"
grep -q '@fission-ai/openspec@latest' "$ROOT_DIR/modules/openspec/module.sh"
grep -q 'npm install -g opencode-ai' "$ROOT_DIR/modules/opencode/module.sh"
for command in '--profile developer --dry-run' '--new meu-projeto --stack node' '--new . --stack node' 'ai-dev install python --dry-run'; do
  grep -q -- "$command" "$ROOT_DIR/README.md"
done

if grep -R -n -E --exclude='audit.sh' --exclude='contracts.sh' --exclude-dir='projeto' 'ghp_|github_pat_|sk-[A-Za-z0-9]{16}|xox[baprs]-|AKIA[0-9A-Z]{16}|AIza[0-9A-Za-z_-]{35}' "$ROOT_DIR"; then
